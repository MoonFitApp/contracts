...
//import "@openzeppelin/contracts-contract-upgradeable/interfaces/IERC721Upgradeable.sol";
...
interface MoonBeast {
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) external returns (uint256);

    function setApprovedOperator(address operator, bool isApproved) external;

    function withdrawNFT(uint256 tokenId, string memory _tokenURI, address receiver) external returns (uint256);
}

contract UMoonFitNFTSale is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;

    IERC721 public _mintPassNFT;
    MoonBeast public _moonBeastNFT;
    ...
    function initialize(uint256 maxSaleAmount, uint256 price, uint16 nftPerPass, address mintPassNFT, address moonBeastNFT, address wallet) initializer public {
        require(maxSaleAmount > 0, "maxSaleAmount must be greater than 0");
        require(price > 0, "maxSaleAmount must be greater than 0");
        require(nftPerPass > 0, "maxSaleAmount must be greater than 0");
        require(mintPassNFT != address(0), "mintPassNFT is the zero address");
        require(moonBeastNFT != address(0), "moonBeastNFT is the zero address");
        ...
        _moonBeastNFT = MoonBeast(moonBeastNFT);
    ...
    function buyNFT(uint256[] calldata mintPassIds, uint16 amount) public nonReentrant payable {
        ...
        require(amount > 0, "maxSaleAmount must be greater than 0");
        ...
        for (uint256 i = 0; i < mintPassIds.length; i++) {
            ...
            for (uint16 j = 0; j < _nftPerPass; j++) {
                ...
                if (_boughtNFT < amount && _usedMintPass[mintPassId] < _nftPerPass) {
                    uint256 tokenId = _moonBeastNFT.mintNFT(beneficiary);
    ...
    function setMoonBeastNFT(address moonBeast) external onlyOwner {
        require(moonBeast != address(0), "MoonBeastNFT is the zero address");
        _moonBeastNFT = MoonBeast(moonBeast);
    }

    function _preValidatePurchase(uint256 amount, address beneficiary, uint256 weiAmount) internal view {
        require(amount > 0, "You need at least one mint pass to mint NFT");
        require(beneficiary != address(0), "Beneficiary is the zero address");
        require(_price * amount == weiAmount, "The value you sent is invalid");
    }
